...
def main():
    ...
    anchor_generator = AnchorGeneratorWithAnchorShape(
        feature_map_scales=[2**l for l in range(len(args.returned_layers) + 1)], ## [1,2,4]
        base_anchor_shapes=args.base_anchor_shapes,
    )
    # import pdb;pdb.set_trace()
    # 2) build network
    conv1_t_size = [max(7, 2 * s + 1) for s in args.conv1_t_stride] ### [7,7,7]第一层卷积的卷积核
    ...
    feature_extractor = resnet_fpn_feature_extractor(
        backbone=backbone,
        spatial_dims=args.spatial_dims, ## 3
        pretrained_backbone=False,
        trainable_backbone_layers=None,
        returned_layers=args.returned_layers,
    )
    num_anchors = anchor_generator.num_anchors_per_location()[0] ## 3
    size_divisible = [s * 2 * 2 ** max(args.returned_layers) for s in feature_extractor.body.conv1.stride]  ## [16,16,8]  s in [2,2,1](args.conv1_t_stride)
    net = torch.jit.script(
        RetinaNet(
            spatial_dims=args.spatial_dims, ## 3
            num_classes=len(args.fg_labels), ## 1
            num_anchors=num_anchors, ## 3
            feature_extractor=feature_extractor,
            size_divisible=size_divisible, ## [16.16,8]
        )
    )
    ...
    for epoch in range(max_epochs):
        ...
        # Training
        for batch_data in train_loader:  ##batch_size=4 list(batch_data[0][0].keys())=['box', 'image', 'label', 'image_meta_dict']
            step += 1
            inputs = [
                batch_data_ii["image"].to(device) for batch_data_i in batch_data for batch_data_ii in batch_data_i
            ]  ## inputs[0].shape=torch.Size([1, 192, 192, 80])   len(inputs) = batch_size=4
            targets = [
                dict(
            ...
            if amp and (scaler is not None):
                with torch.cuda.amp.autocast():
                    ...
                    # {'classification': metatensor(0.4529, device='cuda:0', dtype=torch.float16, grad_fn=<AliasBackward0>), 
                    #  'box_regression': metatensor(0.5483, device='cuda:0', dtype=torch.float16, grad_fn=<AliasBackward0>)}
...
## CUDA_VISIBLE_DEVICES=3 python3 luna16_training.py -e ./config/environment_luna16_fold0.json -c ./config/config_train_luna16_16g.json

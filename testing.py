...
def main():
    ...
    with torch.no_grad():
        ...
        for inference_data in inference_loader:
            #!list(inference_data[0].keys()) = ['box', 'image', 'label', 'image_meta_dict']
            inference_img_filenames = [
                inference_data_i["image_meta_dict"]["filename_or_obj"] for inference_data_i in inference_data
            ]
            print(inference_img_filenames) ## ['xxx.nii']
            use_inferer = not all(
                [inference_data_i["image"][0, ...].numel() < np.prod(patch_size) for inference_data_i in inference_data] ## [True]  not all([True])=False
            ) ## inference_data_i["image"][0, ...].shape=torch.Size([427, 427, 236])   inference_data_i["image"].shape=torch.Size([1, 427, 427, 236])
            inference_inputs = [inference_data_i["image"].to(device) for inference_data_i in inference_data]
            # inference_inputs[0].shape   len(inference_inputs)=1
            # torch.Size([1, 427, 427, 236])
